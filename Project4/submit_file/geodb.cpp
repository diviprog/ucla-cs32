...
GeoDatabase::GeoDatabase()
    : GeoDatabaseBase(), poiMap(0.75), // Initialize with the default load factor
      connectionsMap(0.75),
      streetNamesMap(0.75) {}
...
std::string GeoDatabase::get_street_name(const GeoPoint& pt1, const GeoPoint& pt2) const {
    std::string key = pt1.to_string() + "-" + pt2.to_string(); // Example key format
    const std::string* streetName = streetNamesMap.find(key);
    if (streetName) {
        return *streetName;
    }
    return "";
